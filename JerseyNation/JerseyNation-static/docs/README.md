...
## 📝 Pending follow-up fixes in the Razor version

- Adjust all chart thousand separators to the U.S. format with commas.
- Fix the Read page table alignment when a jersey name wraps to two lines, so the action column remains aligned with the rest of the row.
